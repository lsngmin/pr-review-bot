...
- **두 LLM 교차 검증**: 리뷰는 OpenAI(`gpt-5.4`), 검증은 Anthropic(`claude-sonnet-4-6`). 같은 모델이 자기 출력을 옹호하는 편향 차단
- **환각 라인 차단**: LLM이 지목한 라인이 실제 diff hunk 안에 없으면 인라인 게시 보류. 단일·다중 라인 범위 모두 연속성 검증
- **동시 트리거 atomic 차단**: `ConcurrentHashMap.compute` 단일 원자 호출로 같은 commit SHA 중복 리뷰를 정확히 1회만 실행
- **단일 JSON Schema 양 LLM 공유**: OpenAI Structured Outputs(strict)와 Anthropic Tool Use가 같은 schema 본문 재사용 — LLM 추가 시 schema 코드 중복 0
...
sequenceDiagram
    ...
    participant GH as GitHub
    participant Bot as Pawranoid
    participant LLM as 검증자 LLM (Claude)

    User->>GH: 봇 인라인 코멘트 답글 "@pawranoid verify"
    GH->>Bot: pull_request_review_comment webhook
    Bot->>Bot: 자기 답글 / 비-답글 / 패턴 미일치 컷
    Bot->>GH: 부모 코멘트 + 주변 코드 ±20줄 조회
    GH-->>Bot: 원본 코멘트 + diff hunk + surrounding
    Bot->>Bot: 부모 작성자가 봇인지 확인, 동시 verify 차단
    Bot->>LLM: 원본 코멘트 + 코드 컨텍스트 + 검증 프롬프트
    LLM-->>Bot: 자유 텍스트 의견
...
- **단일 `LlmPort` 인터페이스 + 다중 구현**: `@Primary` 로 `GptClient` 를 메인 리뷰 LLM 으로, `@Qualifier("claudeClient")` 로 `ClaudeClient` 를 verifier 에 강제 주입. LLM 추가/교체 시 application 코드 변경 없음 — [`LlmPort.kt`](src/main/kotlin/com/pawranoid/domain/port/LlmPort.kt)

- **단일 JSON Schema 양 LLM 재사용**: OpenAI Structured Outputs (`response_format: json_schema`, strict) 와 Anthropic Tool Use (`tool input_schema`) 가 같은 schema 본문을 사용. 각 클라이언트는 wrapping 만 다르게 적용 — [`ReviewSchema.kt`](src/main/kotlin/com/pawranoid/infrastructure/llm/ReviewSchema.kt)

### PR 처리 파이프라인

- **diff hunk 라인 검증**: LLM 이 지목한 `line` 이 patch hunk 의 실제 라인에 존재할 때만 인라인 코멘트로 변환. 다중 라인은 `startLine..line` 범위 **전체**가 hunk 안에 있어야 통과 (GitHub API 가 중간 비는 범위는 422 거부) — [`ReviewService.kt:isIssueInDiff`](src/main/kotlin/com/pawranoid/domain/service/ReviewService.kt)
...
JUnit 5 + AssertJ 단위 테스트 101개 통과. 외부 의존성(LLM/GitHub)은 격리하고 순수 로직만 검증.
...
### 4. 왜 단일 JSON Schema 인가?

OpenAI Structured Outputs 와 Anthropic Tool Use 는 표면이 다르다.

- OpenAI: `response_format: { type: "json_schema", strict: true, schema: {...} }`
- Anthropic: `tools: [{ name: "...", input_schema: {...} }]` + `tool_choice` 강제

본문 schema 자체는 JSON Schema spec 이라 구조가 같다. 두 곳에 같은 schema 를 따로 적으면 한쪽 변경 시 다른 쪽 빠짐이 생긴다. 그래서 `ReviewSchema.ROOT` 한 본문을 정의하고 양 클라이언트가 wrapping 만 다르게 적용하도록 분리했다.

```kotlin
// GptClient
"response_format" to mapOf(
    "type" to "json_schema",
    "json_schema" to mapOf("name" to "review", "strict" to true, "schema" to ReviewSchema.ROOT)
)

// ClaudeClient
"tools" to listOf(mapOf("name" to "submit_review", "input_schema" to ReviewSchema.ROOT))
```

새 LLM 추가할 때 schema 는 그대로 재사용, wrapping 만 1곳에서 추가하면 끝. OpenAI strict mode 가 모든 properties 를 required 로 강제하는 제약은 nullable union 타입(`["string", "null"]`) 으로 우회.
